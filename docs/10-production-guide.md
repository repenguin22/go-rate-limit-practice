...
### 2.5 認証基盤(Auth0 等)と組み合わせてプラン別の制限をかける

「入口は gateway のみ + 認証済みクライアント単位」の構成(A だけでよい構成)での
定石。Auth0 の例:

1. **key は検証済みトークンのクレームから取る。** M2M クライアントなら
   `azp`(client_id)、ユーザートークンなら `sub`。JWT 検証ミドルウェア →
   レートリミットミドルウェアの順に並べ、KeyFunc は context 上の検証済み
   claims だけを見る
2. **プラン(tier)は Application metadata → カスタムクレームで運ぶ。**
   リクエストごとに Auth0 Management API へ metadata を取りに行っては
   いけない(Management API 自体に厳しいレート制限がある)。Auth0 Action で
   トークン発行時に `client_metadata` の tier を名前空間付きカスタムクレームに
   焼き込めば、gateway は追加通信ゼロで tier を知れる。反映がトークン
   有効期限まで遅延するのがトレードオフ
3. **tier ごとの制限は [ratelimit.TierLimiter](../ratelimit/tier.go) で振り分ける。**
   tier ごとに Limiter(制限値違い)を用意し、KeyFunc が
   `ratelimit.TierKey(claims.RateTier, claims.ClientID)` を返すだけで、
   ミドルウェアは無改造で動く:

```go
free, _ := redislimit.NewTokenBucket(rdb, 60, time.Minute,
	redislimit.WithKeyPrefix("rl:free:"))
pro, _ := redislimit.NewTokenBucket(rdb, 600, time.Minute,
	redislimit.WithKeyPrefix("rl:pro:"))
lim, _ := ratelimit.NewTierLimiter(
	map[string]ratelimit.Limiter{"free": free, "pro": pro},
	free, // 未知 tier は最も厳しい側に倒す(fallback は必須)
)
mw, _ := middleware.New(lim, keyFromClaims,
	middleware.WithFailurePolicy(middleware.FailOpen))
```

   fallback を必須にしているのは設計判断: 「未知 tier は無制限」は設定ミス
   一つで制限が消え、「未知 tier は全拒否」は設定ミス一つで障害になる。
   安全側の既定を呼び出し側に明示させる。
   tier はクライアントの自己申告ではなく**検証済みクレーム**から取ること
   (自称 tier を信じると、緩いプランを名乗るだけですり抜けられる)
